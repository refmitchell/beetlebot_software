...
for b in range(3):
    ...
    for topic, msg, t in messages:
        if topic == 'yaw':
            ...
            pols[bagfile].append(msg.data)
            counter+=1

    bag.close()

# Init plot structures
...
for k in keys:
    ...
    pd_1 = [x for (x,_,_,_) in pols[k]]
    pd_2 = [x for (_,x,_,_) in pols[k]]
    pd_3 = [x for (_,_,x,_) in pols[k]]
    pd_4 = [x for (_,_,_,x) in pols[k]]
